...
CREATE OR ALTER FUNCTION YIN.sfStringCompare(@StringCompare VARCHAR(500)) RETURNS INT AS

BEGIN
   DECLARE @i INT = 1
   DECLARE @Output INT = 0
   DECLARE @x VARCHAR(1)
   DECLARE @StringLength INT = LEN(@StringCompare)/2
   DECLARE @StringLeft VARCHAR(500) = LEFT(@StringCompare,@StringLength)
   DECLARE @StringRight VARCHAR(500) = RIGHT(@StringCompare,@StringLength)

   WHILE @i <= @StringLength
   BEGIN
      SET @x = SUBSTRING(@StringLeft,@i,1)
      IF CHARINDEX(@x, @StringRight COLLATE LATIN1_GENERAL_CS_AS) > 0
      BEGIN
...
SELECT SUM(NEXUS_DW.YIN.sfStringCompare(Blah)) MyPriority
  FROM MY_BLAH
;
